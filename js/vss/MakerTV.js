const path = require('path');
const videostuff = require('../../js/videostuff');
const um = require('../../js/upm');
const globals = require("../globals");

exports.getHost = function() {
    return "http://www.maker.tv/";
};

exports.getFavicon = function() {
    return "data:image/png;base64,";
}

exports.getId = function(url) {
    if (!globals.isValidUrl(url)) return "";
	var matches = url.match(/(maker\.tv|makerplayer\.com)\/.*?(maker|video)\/([A-Z0-9]{12})/i);
	if (matches == null) return "";
	return matches[3];
};

var addQuality = function(qual, info, count, cb) {
    var quality = new videostuff.videoQuality();
    var isAudio = false;
    var upm = new um(qual.file);
    if (qual.height == -1) {

    	// Audio file
    	isAudio = true;
		quality = new videostuff.audioQuality();
        quality.kbits = -1;
    } else {
    	quality.progScan = qual.height + "p";
    }
    quality.downloadUrl = qual.file;
    quality.type = path.extname(upm.host);

    // Get file size
    globals.getFileSize(quality.downloadUrl, function(size) {
        quality.size = size;
        if (!isAudio) {
            info.videoQualities.push(quality);
        } else {
            info.audioQualities.push(quality);
        }
        if (count == info.videoQualities.length + info.audioQualities.length) {
            cb(info);
        }
    });
}

exports.getInfo = function(url, cb) {
    var info = new videostuff.downloadInfo();
    info.url = url;
    info.id = exports.getId(url);
    info.videoQualities = [];
    info.audioQualities = [];
    globals.getSource("http://www.maker.tv/video/" + info.id, function(err, resp, body) {
        info.isValid = body.indexOf("<div data-jwid=") > -1;

        if (info.isValid) {
            var mediaId = globals.inBetween(body, '<div data-jwid="', '" class="player-wrap">');

            //Get Json source and parse it to get qualities and video info.
            globals.getSource("http://content.jwplatform.com/feeds/" + mediaId + ".json", function(err, resp, body) {
                var jInfo = JSON.parse(body).playlist[0];

                info.thumbUrl = "http://content.jwplatform.com/thumbs/" + mediaId + "-480.jpg"
                info.title = jInfo.title;
                info.uploader = "";
                info.viewCount = -1;
                info.duration = -1;

                var qualities = [];
                for (var j = 0; j < jInfo.sources.length; j++) {
                    var qual = jInfo.sources[j];
                    if (qual.duration == null) continue;
                    if (info.duration == -1) info.duration = qual.duration;
                    qual.file = "http:" + qual.file;
                    qualities.push(qual);
                }

                for (var i = 0; i < qualities.length; i++) {
                    addQuality(qualities[i], info, qualities.length, cb);
                }
            });
        } else {
            cb(info);
        }
    });
};

exports.validateUrl = function(url) {
    return exports.getId(url) != "";
};
